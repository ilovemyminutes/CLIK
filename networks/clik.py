...
# NOTE Ablation - Listwise Ranking Loss
class Ablation2(_CLIK):
    def __init__(
        self,
        feature_dim: int,
        queue_size: int,
    ...
    def get_preference_discrimination_result(
        ...
        if discrim["instances"].ndim == 5:  # [D, K, C, H, W]
            logits, energy = [], []
            for context_row, instances_row, scores_row in zip(
                contexts.unbind(dim=0), discrim["instances"].unbind(dim=0), discrim['scores'].unbind(dim=0)
            ):
                context_row = context_row.unsqueeze(0)
                energy_row = F.softmax(torch.mm(context_row, self.inst_queue.T), dim=1)
                virt_instance = F.normalize(torch.mm(energy_row, self.inst_queue))
            ...
            logits = torch.mm(joint_emb, instances.T)
            labels = discrim['scores']

        output = [logits, labels]
        if return_loss:  # loss should be calculated in forward() during DDP
            output.append(self.listwise_ranking_loss(logits, labels))
        if return_energy:  # NOTE. mainly for debugging
            if isinstance(energy, list):
                energy = torch.vstack(energy)
            output.append(energy)
    ...
    def listwise_ranking_loss(self, logits: torch.Tensor, labels: torch.Tensor):
        logits = F.softmax(logits, dim=-1)
        labels = F.softmax(labels / self.temperature, dim=-1)
        loss = -torch.sum(labels*torch.log(logits))
        return loss